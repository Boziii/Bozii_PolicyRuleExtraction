...
class ruleExtractor:
    ...
    def make_rule_trees(self, tree, featureList, class_names):
        ...
        if tree_.feature[0] == _tree.TREE_UNDEFINED:
            rootNode = ruleTreeNode(np.argmax(tree_.value[0]))
        else:
            featVal = tree_.feature[0]
            threshold = tree_.threshold[0]
            rootNode = ruleTreeNode(conditionClass(featureList[featVal], threshold, True))
...
#         
        def combineBranches(target, targetRules):
            ...
            return ruleTreeVar         
#         
#         def addBranchNode(currNode, addedNode, currentDepth, nodeDepth, isTarget, conditionIsTrue):  
#             ruleTreeVar = [] 
#             if(currNode == addedNode and nodeDepth == currentDepth):
#                 return
#             elif ()
#          
